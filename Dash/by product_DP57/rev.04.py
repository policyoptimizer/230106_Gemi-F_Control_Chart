# 시험규격 추가
# DP57 Assay : 93.0~102.0%
# DP57 Chiral Purity : 97.0%ee 이상
# DP57 Organic Impurities Total : 5.0% 이하
...
def load_data_and_create_graphs(dataset_name, window_size, sigma_level, recent_batches, trend_threshold, excluded_batches):
    ...
    # 시험규격 설정
    assay_spec_lower = 93.0
    assay_spec_upper = 102.0
    chiral_spec = 97.0
    total_impurity_spec = 5.0
    ...
    # 각 시험 항목에 대한 규격선 추가
    if dataset_name == "DP57_Assay":
        trend_fig.add_trace(go.Scatter(x=df[batch_column], y=[assay_spec_lower]*len(df), mode='lines', name='Spec Lower 93.0%', line=dict(color='red', dash='dot')))
    ...
    elif dataset_name == "DP57_Chiral":
        trend_fig.add_trace(go.Scatter(x=df[batch_column], y=[chiral_spec]*len(df), mode='lines', name='Spec 97.0%ee 이상', line=dict(color='red', dash='dot')))
    elif dataset_name == "DP57_Total_Impurity":
        trend_fig.add_trace(go.Scatter(x=df[batch_column], y=[total_impurity_spec]*len(df), mode='lines', name='Spec 5.0% 이하', line=dict(color='red', dash='dot')))
    ...
    # 각 시험 항목에 대한 규격선 추가
    if dataset_name == "DP57_Assay":
        anomaly_fig.add_trace(go.Scatter(x=df[batch_column], y=[assay_spec_lower]*len(df), mode='lines', name='Spec Lower 93.0%', line=dict(color='red', dash='dot')))
        ...
        anomaly_fig.add_trace(go.Scatter(x=df[batch_column], y=[chiral_spec]*len(df), mode='lines', name='Spec 97.0%ee 이상', line=dict(color='red', dash='dot')))
    elif dataset_name == "DP57_Total_Impurity":
        anomaly_fig.add_trace(go.Scatter(x=df[batch_column], y=[total_impurity_spec]*len(df), mode='lines', name='Spec 5.0% 이하', line=dict(color='red', dash='dot')))
    ...
    summary_table = html.Table([
        ...
        html.Tbody([
            ...
            # 시험규격 추가
            html.Tr([html.Td("DP57 Assay 시험규격", style={'text-align': 'center'}), html.Td("93.0~102.0%", style={'text-align': 'center'})]) if dataset_name == "DP57_Assay" else None,
            html.Tr([html.Td("DP57 Chiral 시험규격", style={'text-align': 'center'}), html.Td("97.0%ee 이상", style={'text-align': 'center'})]) if dataset_name == "DP57_Chiral" else None,
            html.Tr([html.Td("DP57 Total Impurity 시험규격", style={'text-align': 'center'}), html.Td("5.0% 이하", style={'text-align': 'center'})]) if dataset_name == "DP57_Total_Impurity" else None
...
# DP57 제품에 대한 탭 생성 함수
def create_dp57_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold):
...
def create_dp57_tabs(window_size, sigma_level, recent_batches, trend_threshold):
    dp57_tabs = []
    for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]:
        dp57_tabs.append(create_dp57_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold))
    return dp57_tabs

# 앱 레이아웃 설정
app.layout = html.Div([
    html.H1('CQA DP57 Assay, Chiral, Total Impurity'),
    dcc.Tabs(id="dp57-tabs", children=create_dp57_tabs(5, 1, 5, 0.02)),
    html.Div([
        html.H3('넬슨 법칙'),
...
@app.callback(
    [Output(f'summary-table-{dataset}', 'children') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [Output(f'trend-graph-{dataset}', 'figure') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [Output(f'anomaly-graph-{dataset}', 'figure') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]],
    [Input(f'window-size-dropdown-{dataset}', 'value') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [Input(f'sigma-level-dropdown-{dataset}', 'value') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [Input(f'recent-batches-dropdown-{dataset}', 'value') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [Input(f'trend-threshold-dropdown-{dataset}', 'value') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [Input(f'exclude-button-{dataset}', 'n_clicks') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]] +
    [State(f'excluded-batches-{dataset}', 'value') for dataset in ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"]]
)
def update_tabs(*args):
    window_sizes = args[:3]
    sigma_levels = args[3:6]
    recent_batches = args[6:9]
    trend_thresholds = args[9:12]
    n_clicks_list = args[12:15]
    excluded_batches_list = args[15:]
    ...
    for dataset, window_size, sigma_level, recent_batch, trend_threshold, n_clicks, excluded_batches in zip(
        ["DP57_Assay", "DP57_Chiral", "DP57_Total_Impurity"],
