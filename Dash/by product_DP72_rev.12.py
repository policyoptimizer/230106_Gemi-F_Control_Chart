# - 문제 : 최근 배치 수 n' 과 '경향성 판단 Threshold' 값이 업데이트되지 않음
# - 원인 : 콜백 함수의 인덱싱 및 매핑 문제로 인함
# - 솔루션 : 
#   콜백 함수의 인덱싱을 더 명확히 함 
#   각 인풋 값이 올바른 데이터셋에 매핑되는지 확인
#   콜백 함수의 인덱스 매핑이 일관되게 처리
#   콜백 함수 update_tabs에서 인덱스를 명확하게 처리하여 
#     각 입력 값이 올바른 데이터셋에 매핑되도록 함
...
def update_tabs(*args):
   ...
   trend_figs = []
   anomaly_figs = []
   for i, dataset in enumerate(["DP67_AUI", "DP67_Total_Impurity"]):
       trend_fig, anomaly_fig, summary_table = load_data_and_create_graphs(
           dataset, window_sizes[i], sigma_levels[i], recent_batches[i], trend_thresholds[i]
       )
