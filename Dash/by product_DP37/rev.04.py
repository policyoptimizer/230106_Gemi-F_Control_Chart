...
def load_data_and_create_graphs(dataset_name, window_size, sigma_level, recent_batches, trend_threshold, excluded_batches):
    ...
    summary_table = html.Table([
        ...
        html.Tbody([
            ...
            # 시험규격 추가
            html.Tr([html.Td("DP37 Assay 시험규격", style={'text-align': 'center'}), html.Td("Information Only", style={'text-align': 'center'})])
...
# DP37 제품에 대한 탭 생성 함수
def create_dp37_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold):
...
def create_dp37_tabs(window_size, sigma_level, recent_batches, trend_threshold):
    dp37_tabs = []
    for dataset in ["DP37_Assay"]:
        dp37_tabs.append(create_dp37_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold))
    return dp37_tabs

# 앱 레이아웃 설정
app.layout = html.Div([
    html.H1('CQA DP37 Assay'),
    dcc.Tabs(id="dp37-tabs", children=create_dp37_tabs(5, 1, 5, 0.02)),
    html.Div([
        html.H3('넬슨 법칙'),
...
@app.callback(
    [Output(f'summary-table-{dataset}', 'children') for dataset in ["DP37_Assay"]] +
    [Output(f'trend-graph-{dataset}', 'figure') for dataset in ["DP37_Assay"]] +
    [Output(f'anomaly-graph-{dataset}', 'figure') for dataset in ["DP37_Assay"]],
    [Input(f'window-size-dropdown-{dataset}', 'value') for dataset in ["DP37_Assay"]] +
    [Input(f'sigma-level-dropdown-{dataset}', 'value') for dataset in ["DP37_Assay"]] +
    [Input(f'recent-batches-dropdown-{dataset}', 'value') for dataset in ["DP37_Assay"]] +
    [Input(f'trend-threshold-dropdown-{dataset}', 'value') for dataset in ["DP37_Assay"]] +
    [Input(f'exclude-button-{dataset}', 'n_clicks') for dataset in ["DP37_Assay"]] +
    [State(f'excluded-batches-{dataset}', 'value') for dataset in ["DP37_Assay"]]
)
def update_tabs(*args):
    window_sizes = args[:1]
    sigma_levels = args[1:2]
    recent_batches = args[2:3]
    trend_thresholds = args[3:4]
    n_clicks_list = args[4:5]
    excluded_batches_list = args[5:]
