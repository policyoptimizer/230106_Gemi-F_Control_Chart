# 요약 보고서 위에 아래 2가지 설정할 수 있게 함

# 최근 배치 수 설정
# Historical 평균 대비 Threshold 설정

# 다른 탭에서 최근 배치 수, 경향성 판단 threshold 값 업데이트 안됨
...
# DP72 제품에 대한 탭 생성 함수
def create_dp72_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold):
...
def create_dp72_tabs(window_size, sigma_level, recent_batches, trend_threshold):
   dp72_tabs = []
   for dataset in ["DP72_Assay", "DP72_Chiral", "DP72_AUI", "DP72_Total_Impurity", "DP72_ROI", "DP72_Impurity-1"]:
       dp72_tabs.append(create_dp72_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold))
   return dp72_tabs
...
def update_tabs(*args):
   ...
   for dataset, window_size, sigma_level, recent_batch, trend_threshold in zip(
       ...
       dp72_tabs.append(create_dp72_tab(dataset, window_size, sigma_level, recent_batch, trend_threshold))
   return dp72_tabs
