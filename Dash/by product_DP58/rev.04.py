# 시험규격 추가
# DP58 Assay : 95.0~102.0%
# DP58 Chiral Purity : 96.0%ee 이상
...
def load_data_and_create_graphs(dataset_name, window_size, sigma_level, recent_batches, trend_threshold, excluded_batches):
    ...
    # 각 시험 항목에 대한 규격선 추가
    if dataset_name == "DP58_Assay":
        trend_fig.add_trace(go.Scatter(x=df[batch_column], y=[assay_spec_lower]*len(df), mode='lines', name='Spec Lower 95.0%', line=dict(color='red', dash='dot')))
    ...
    elif dataset_name == "DP58_Chiral":
        trend_fig.add_trace(go.Scatter(x=df[batch_column], y=[chiral_spec]*len(df), mode='lines', name='Spec 96.0%ee 이상', line=dict(color='red', dash='dot')))
    ...
    # 각 시험 항목에 대한 규격선 추가
    if dataset_name == "DP58_Assay":
        anomaly_fig.add_trace(go.Scatter(x=df[batch_column], y=[assay_spec_lower]*len(df), mode='lines', name='Spec Lower 95.0%', line=dict(color='red', dash='dot')))
        ...
        anomaly_fig.add_trace(go.Scatter(x=df[batch_column], y=[chiral_spec]*len(df), mode='lines', name='Spec 96.0%ee 이상', line=dict(color='red', dash='dot')))
    ...
    summary_table = html.Table([
        ...
        html.Tbody([
            ...
            # 시험규격 추가
            html.Tr([html.Td("DP58 Assay 시험규격", style={'text-align': 'center'}), html.Td("95.0~102.0%", style={'text-align': 'center'})]) if dataset_name == "DP58_Assay" else None,
            html.Tr([html.Td("DP58 Chiral Purity 시험규격", style={'text-align': 'center'}), html.Td("96.0%ee 이상", style={'text-align': 'center'})]) if dataset_name == "DP58_Chiral" else None
