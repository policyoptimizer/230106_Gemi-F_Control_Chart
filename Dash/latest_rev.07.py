...
def get_recent_batches_and_stats(df, num_batches):
    recent_batches = df.sort_values(by='Batch').tail(num_batches)
    stats = {
...
def filter_by_year(df, start_year, end_year):
    df['Year'] = df['Batch'].apply(lambda x: int(x[1:3]) if x[0] == 'M' else int(x[3:5]))
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Button('CSV 다운로드', id=f'download-button-{dataset}', n_clicks=0),
