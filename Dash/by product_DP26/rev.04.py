# 시험규격 추가
# DP26 Assay : Information Only
# DP26 Triester : Information Only 0.5% 이하
...
def load_data_and_create_graphs(dataset_name, window_size, sigma_level, recent_batches, trend_threshold, excluded_batches):
    ...
    # DP26 Triester 불순물 규격선 추가
    if dataset_name == "DP26_Triester":
        trend_fig.add_trace(go.Scatter(x=df[batch_column], y=[0.5]*len(df), mode='lines', name='Spec Limit 0.5%', line=dict(color='red', dash='dot')))
    ...
    # DP26 Triester 불순물 규격선 추가
    if dataset_name == "DP26_Triester":
        anomaly_fig.add_trace(go.Scatter(x=df[batch_column], y=[0.5]*len(df), mode='lines', name='Spec Limit 0.5%', line=dict(color='red', dash='dot')))
    ...
    summary_table = html.Table([
        ...
        html.Tbody([
            ...
            # 규격 추가
            html.Tr([html.Td("Assay 시험규격", style={'text-align': 'center'}), html.Td("Information Only", style={'text-align': 'center'})]) if dataset_name == "DP26_Assay" else None,
            html.Tr([html.Td("Triester 불순물 시험규격", style={'text-align': 'center'}), html.Td("Information Only (0.5% 이하)", style={'text-align': 'center'})]) if dataset_name == "DP26_Triester" else None
...
@app.callback(
    ...
    [Input(f'exclude-button-{dataset}', 'n_clicks') for dataset in ["DP26_Assay", "DP26_Triester"]] +
    [State(f'excluded-batches-{dataset}', 'value') for dataset in ["DP26_Assay", "DP26_Triester"]]
)
