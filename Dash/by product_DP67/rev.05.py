...
def load_data_and_create_graphs(dataset_name, window_size, sigma_level, recent_batches, trend_threshold, excluded_batches, location_filter):
    ...
    elif dataset_name == "DP67_Total_Impurity":
    ...
    summary_table = html.Table([
        ...
        html.Tbody([
            ...
            # 시험규격 추가
            html.Tr([html.Td("DP67 AUI 시험규격", style={'text-align': 'center'}), html.Td("0.1% 이하", style={'text-align': 'center'})]) if dataset_name == "DP67_AUI" else None,
            html.Tr([html.Td("DP67 Total Impurity 시험규격", style={'text-align': 'center'}), html.Td("2.0% 이하", style={'text-align': 'center'})]) if dataset_name == "DP67_Total_Impurity" else None
...
# DP67 제품에 대한 탭 생성 함수
def create_dp67_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold, location_filter):
...
def create_dp67_tabs(window_size, sigma_level, recent_batches, trend_threshold, location_filter):
    dp67_tabs = []
    for dataset in ["DP67_AUI", "DP67_Total_Impurity"]:
        dp67_tabs.append(create_dp67_tab(dataset, window_size, sigma_level, recent_batches, trend_threshold, location_filter))
    return dp67_tabs

# 앱 레이아웃 설정
app.layout = html.Div([
    html.H1('CQA DP67 AUI, Total Impurity'),
    dcc.Tabs(id="dp67-tabs", children=create_dp67_tabs(5, 1, 5, 0.02, "All")),
    html.Div([
        html.H3('넬슨 법칙'),
...
@app.callback(
    [Output(f'summary-table-{dataset}', 'children') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Output(f'trend-graph-{dataset}', 'figure') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Output(f'anomaly-graph-{dataset}', 'figure') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]],
    [Input(f'window-size-dropdown-{dataset}', 'value') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'sigma-level-dropdown-{dataset}', 'value') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'recent-batches-dropdown-{dataset}', 'value') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'trend-threshold-dropdown-{dataset}', 'value') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'exclude-button-{dataset}', 'n_clicks') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [State(f'excluded-batches-{dataset}', 'value') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'all-button-{dataset}', 'n_clicks') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'onsan-button-{dataset}', 'n_clicks') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]] +
    [Input(f'iksan-button-{dataset}', 'n_clicks') for dataset in ["DP67_AUI", "DP67_Total_Impurity"]]
)
def update_tabs(*args):
    ...
    excluded_batches_list = args[10:12]
    all_clicks = args[12:14]
    onasan_clicks = args[14:16]
    iksan_clicks = args[16:]
    ...
    location_filters = []
    for all_click, onsan_click, iksan_click in zip(all_clicks, onasan_clicks, iksan_clicks):
