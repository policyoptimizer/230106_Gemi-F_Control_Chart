...
def load_data_and_create_graphs(dataset_name, window_size):
   ...
   trend_text = "안정" if abs(recent_trend - mean_value) / mean_value < 0.02 else ("상승" if recent_trend > mean_value else "하락")
   ...
   summary_table = html.Table([
       ...
       html.Tbody([
           ...
           html.Tr([html.Td("최근 5배치 경향 : "), html.Td(f"{trend_text} (최근 5배치 평균: {recent_trend:.2f})")])
       ])
   ])
