...
def create_product_tabs(df, window_size, sigma_level, recent_batches, trend_threshold):
    # 제품명을 지정된 순서대로 정렬
    product_order = [
        'Zemiglo', 'Zemidapa', 'Zemimet 25/500mg', 'Zemimet 25/750mg',
        'Zemimet 25/1000mg', 'Zemimet 50/500mg', 'Zemimet 50/1000mg',
        'Zemilow 50/5mg', 'Zemilow 50/10mg', 'Zemilow 50/20mg'
    ]
...
# 데이터 로드 및 앱 레이아웃 설정
dataset = dataiku.Dataset("osong")
df = dataset.get_dataframe()

app.layout = html.Div([
    html.H1('Osong Site Product Quality Analysis'),
...
def generate_callback(product, insp_code):
    ...
    def update_graphs(n_clicks, recent_batches, trend_threshold, sigma_level, window_size, exclude_batches):
        ...
        return trend_fig, anomaly_fig, f'{product} - {insp_code} 요약 보고서'
