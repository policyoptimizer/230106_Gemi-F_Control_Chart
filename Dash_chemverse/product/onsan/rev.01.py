...
def load_data_and_create_graphs(df, product, insp_code, window_size, sigma_level, recent_batches, trend_threshold, excluded_batches):
    ...
    trend_fig.add_trace(go.Scatter(x=df['batch_no'], y=[df['insp_min_value'].iloc[0]]*len(df), mode='lines', name='Spec Lower', line=dict(color='red', dash='dot')))
    ...
    anomaly_fig.add_trace(go.Scatter(x=df['batch_no'], y=[df['insp_min_value'].iloc[0]]*len(df), mode='lines', name='Spec Lower', line=dict(color='red', dash='dot')))
...
def create_product_tab(df, product, window_size, sigma_level, recent_batches, trend_threshold):
    ...
    for insp_code in insp_codes:
        ...
        tabs.append(dcc.Tab(label=insp_code, children=[
            html.Div([
                html.H3(f'{product} - {insp_code} 요약 보고서'),
                summary_table,
                dcc.Graph(figure=trend_fig, config={'clickmode': 'event+select'}),
