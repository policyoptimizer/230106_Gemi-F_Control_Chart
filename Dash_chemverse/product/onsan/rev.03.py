        ...
        tabs.append(dcc.Tab(label=insp_code, children=[
            html.Div([
                ...
                html.Div([
                    ...
                    ], style={'display': 'flex', 'flex-direction': 'row', 'align-items': 'center', 'flex-wrap': 'wrap', 'margin-bottom': '10px'}),
                    html.Div([
                        html.Label('윈도우 사이즈:'),
                        dcc.Dropdown(
                        ...
                        html.Label('제외할 배치 번호:'),
                        dcc.Input(id=f'{product}-{insp_code}-exclude-batches-input', type='text', value=''),
...
def generate_callback(product, insp_code):
    ...
    def update_graphs(n_clicks, recent_batches, trend_threshold, sigma_level, window_size, exclude_batches):
        exclude_batches_list = [int(batch.strip()) for batch in exclude_batches.split(',')] if exclude_batches else []
