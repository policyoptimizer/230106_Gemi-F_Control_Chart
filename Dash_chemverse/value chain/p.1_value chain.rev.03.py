...
def update_content(n_clicks):
    ...
    for node in nodes:
        ...
        if isinstance(df, str):
            ...
            table_rows.append({'Product': product_name, 'Batch': ''})
            continue
        ...
        # 배치 번호 지정
        modify_batch_no = batch_modifications.get(product_name)

        if product_name in ['Zemimet', 'Zemilow']:
            # 서브 제품 처리
            sub_products = {
                'Zemimet': ["Zemimet_50_500mg", "Zemimet_50_1000mg", "Zemimet_25_500mg", "Zemimet_25_1000mg", "Zemimet_25_750mg"],
                'Zemilow': ["Zemilow_50_5mg", "Zemilow_50_10mg", "Zemilow_50_20mg"]
            }[product_name]
            sub_colors = []
            for sub_product in sub_products:
                ...
                if isinstance(sub_df, str):
                    ...
                    sub_product_spec = red_specs.get(sub_product)
                    if not sub_product_spec:
                        sub_colors.append('grey')
                        continue

                    # 해당 배치 번호의 행 찾기
                    idx_modify = sub_df[sub_df['batch_no'] == modify_batch_no].index
                    if not idx_modify.empty:
                        # 검사 결과 값을 허용 기준을 벗어나도록 변경
                        for spec_name in sub_product_spec.keys():
                            red_min, red_max = sub_product_spec[spec_name]
                            if red_min is not None:
                                sub_df.at[idx_modify[0], spec_name] = red_min - 1
                            elif red_max is not None:
                                sub_df.at[idx_modify[0], spec_name] = red_max + 1

                    latest_batch = sub_df[sub_df['batch_no'] == modify_batch_no]
                    if latest_batch.empty:
                        latest_batch = sub_df.iloc[-1:]
                    batch_no = latest_batch['batch_no'].values[0]
                    status_message += f"Batch: {batch_no}\n"
                    batch_statuses = []

                    for spec_name in sub_product_spec:
                        yellow_spec = yellow_specs.get(sub_product, {}).get(spec_name, (None, None))
                        red_spec = red_specs.get(sub_product, {}).get(spec_name, (None, None))
                        if spec_name in latest_batch.columns:
                            value = latest_batch[spec_name].values[0]
            ...
            # 해당 배치 번호의 행 찾기
            idx_modify = df[df['batch_no'] == modify_batch_no].index
            if not idx_modify.empty:
                # 검사 결과 값을 허용 기준을 벗어나도록 변경
                for spec_name in product_spec.keys():
                    red_min, red_max = product_spec[spec_name]
                    if red_min is not None:
                        df.at[idx_modify[0], spec_name] = red_min - 1
                    elif red_max is not None:
                        df.at[idx_modify[0], spec_name] = red_max + 1

            latest_batch = df[df['batch_no'] == modify_batch_no]
            if latest_batch.empty:
            ...
            # 그래프에 필요한 컬럼이 있는지 확인하고 없으면 추가
            if 'insp_result_value' not in df.columns:
                df['insp_result_value'] = 0  # 또는 적절한 값으로 초기화
